#!/usr/bin/env python3
"""Host-runnable CLI producer for the celery-rmq-quorum stack.

Thin shell over ``rabbitmq_client.RabbitMQClient``: argument parsing, batch
iteration, and human-readable output. All AMQP + Celery protocol details
live in ``rabbitmq_client``.

By default connects to ``localhost:5672`` (rabbit1's published port from
docker-compose). Override with ``--broker-url`` or ``$BROKER_URL``.
...
Usage:
    ...
    python producer.py --queue jobs --message "scheduled" --delay-ms 5000
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help=(f"AMQP broker URL (default: {DEFAULT_BROKER_URL}). "
              "Semicolon-separated for multi-host failover. Also reads $BROKER_URL."),
    )
    ap.add_argument(
    ...
    published = 0
    t0 = time.monotonic()
    with RabbitMQClient(args.broker_url, origin="host-producer") as client:
        for item in source:
